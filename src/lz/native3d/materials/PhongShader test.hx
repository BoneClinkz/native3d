		...
		function fragment() {
			if(!isShadowDepth){
				...
				if (shadowProjectonMatrix != null) {
					...
					color2 = shadowMap.get(shadowLightXY);
				}
				if (hasDiffuseMap) {
				...
				out = (comp- comp.xxyz* bitMsk)*0+shadowDepthXYZW.x/shadowDepthZW.y/2+.5;
			}
		}
